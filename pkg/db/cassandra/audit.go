...
func (s *AuditStore) GetOrganization(organizationName string, params db.AuditFilterParams) (types.Audits, types.Error) {
	...
	query := "SELECT " + auditColumns + " FROM audits WHERE organization = ? AND timestamp >= ? AND timestamp <= ? LIMIT ? ALLOW FILTERING"
...
func (s *AuditStore) GetAPIProduct(organizationName, apiproductName string, params db.AuditFilterParams) (types.Audits, types.Error) {
	...
	query := "SELECT " + auditColumns + " FROM audits WHERE organization = ? AND entity_type = ? AND entity_id = ? AND timestamp >= ? AND timestamp <= ? LIMIT ? ALLOW FILTERING"
...
func (s *AuditStore) GetDeveloper(organizationName, developerID string, params db.AuditFilterParams) (types.Audits, types.Error) {
	...
	query := "SELECT " + auditColumns + " FROM audits WHERE organization = ? AND developer_id = ? AND timestamp >= ? AND timestamp <= ? LIMIT ? ALLOW FILTERING"
...
func (s *AuditStore) GetApplication(organizationName, developerID, appID string, params db.AuditFilterParams) (types.Audits, types.Error) {
	...
	query := "SELECT " + auditColumns + " FROM audits WHERE organization = ? AND developer_id = ? AND app_id = ? AND timestamp >= ? AND timestamp <= ? LIMIT ? ALLOW FILTERING"
...
func (s *AuditStore) GetUser(userName string, params db.AuditFilterParams) (types.Audits, types.Error) {
	...
	query := "SELECT " + auditColumns + " FROM audits WHERE user = ? AND timestamp >= ? AND timestamp <= ? LIMIT ? ALLOW FILTERING"
