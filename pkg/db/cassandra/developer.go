...
func (s *DeveloperStore) GetAll(organizationName string) (types.Developers, types.Error) {
	...
	query := s.db.conditionalFiltering("SELECT " + developerColumns + " FROM developers WHERE organization_name = ?")
...
func (s *DeveloperStore) GetByEmail(organizationName, developerEmail string) (*types.Developer, types.Error) {
	...
	query := s.db.conditionalFiltering("SELECT " + developerColumns + " FROM developers WHERE organization_name = ? AND email = ? LIMIT 1")
